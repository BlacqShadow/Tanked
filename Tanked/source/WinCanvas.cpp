...
WinCanvas::WinCanvas(short w, short h, std::wstring s)
	:	Canvas(),
		...
		m_sHeight(h)
{
	/*wcscpy copies a string from the source string buffer to the destination string buffer*/
	/*szClassName is an array of Wide characters defined in WinCanvas.h*/
	wcscpy_s(szClassName,s.c_str());	//Copies a string (destination, size of destination string buffer, source string buffer)
	
	/*A pointer of type wincanvas that stores the current instance of the object. */
	s_pCanvas = this;

	/*
	*	Multithreading being used here - concurrent execution of pieces of the same program
	*	_beginthread - Starts a new thread
	*	_beginthread(start address of the routine that begins the new thread, stack size, argument list to be passed to the new thread)
	*	*IMPORTANT* function call without () == pointer to that function 
	*/	
	_beginthread(OpenWinCanvas, 0, this);

	// checking to see if the canvas is ready for drawing 
	while (!m_bIsReady)
	{
		Sleep(50);
	};
...
{
	RECT wRect;
	//again get the coordianes of the main window and pass it on to the rect structure
...
{
	// A brush is a drawing tool used to fill up closed spaces
	// Using a brush is like taking a bucket of paint and smearing it all over a box or something
	// Burshes come in various varieties and patterns; heck even an image can be used as a brush
	HBRUSH hBrush = CreateSolidBrush(RGB(r,g,b));
	/*
	*	SelectObject function - used to take our brush and select it into our device context
	*	Device context - It encapsulates the windows API's for drawing and such
	*/
	SelectObject(this->m_hdcBuffer,hBrush);

	/*
	*	Create a locgical pen that can be used to draw various curves and lines
	*	Then select it into the device context using SelectObject
	*	CreatePen(Style, width, color)
	*/
	HPEN hPen = CreatePen(PS_SOLID, 1, RGB(r,g,b));
	SelectObject(this->m_hdcBuffer, hPen);

	//Rect structure defines the co-ordinates of upper-left and lower-right corners of a rectangle
	RECT wRect;

	// Retrieves the coordinates of a window's client area
	//	GetClientRect(the window whose co-ordinates are requried, RECT Structure that recieves the co-ordinates)
	//	Here the top left and co-ordinates are (0,0) and the bottom co-ordinates are the length of the window)
	GetClientRect(this->m_hWnd, &wRect);

	//	FillRect - takes the RECT structure and fills it with the brush specified
	//	FillRect(Device Context, RECT structure, Brush)
...
void WinCanvas::Size(unsigned short x, unsigned short y)
{
	RECT wRect;
...
void WinCanvas::DrawPoly(Polytype const & p, unsigned long colour, unsigned int width)
{
	switch (p.type())
	{
		case POLYGON_TYPE::CIRCLE:
			DrawCircle(dynamic_cast<Circle const &>(p), colour, width);
			break;

		case POLYGON_TYPE::RECTANGLE:
			DrawRect(dynamic_cast<Rect const &>(p), colour, width);
			break;

		case POLYGON_TYPE::TRIANGLE:
			DrawTriangle(dynamic_cast<Triangle const &>(p), colour, width);
			break;
	}
}

void WinCanvas::DrawFilledPoly(Polytype const & p, unsigned long fill_colour, unsigned long line_colour, unsigned int width)
{
	switch (p.type())
	{
		case POLYGON_TYPE::CIRCLE:
			DrawSolidCircle(dynamic_cast<Circle const &>(p), fill_colour, line_colour, width);
			break;

		case POLYGON_TYPE::RECTANGLE:
			DrawSolidRect(dynamic_cast<Rect const &>(p), fill_colour, line_colour, width);
			break;

		case POLYGON_TYPE::TRIANGLE:
			DrawSolidTriangle(dynamic_cast<Triangle const &>(p), fill_colour, line_colour, width);
			break;
	}
}
...
void WinCanvas::DrawRect(Rect const & r, unsigned long line_rgb, int line_width)
{
	...
	// create the set of points
	POINT pts[4 + 1];
	pts[0].x = (long)r.start().x;
	pts[0].y = (long)r.start().y;
	pts[1].x = (long)r.start().x;
	pts[1].y = (long)r.end().y;
	pts[2].x = (long)r.end().x;
	pts[2].y = (long)r.end().y;
	pts[3].x = (long)r.end().x;
	pts[3].y = (long)r.start().y;
	pts[4].x = (long)r.start().x;
	pts[4].y = (long)r.start().y;
...
// -------------------------------------------------------------------------
void WinCanvas::DrawTexture(Texture const & t, POINT2 pos, POINT2 s)
{
...
void WinCanvas::Write(int x, int y, std::string text, unsigned long colour)
{
	HBRUSH NewBrush = CreateSolidBrush(colour);
	SelectObject(m_hdcBuffer, NewBrush); 
	HPEN NewPen = CreatePen (PS_SOLID, 1, colour);
	SelectObject(m_hdcBuffer, NewPen); 

	// Sets the color of the specified device context to the specified color
	SetTextColor(m_hdcBuffer, colour);
